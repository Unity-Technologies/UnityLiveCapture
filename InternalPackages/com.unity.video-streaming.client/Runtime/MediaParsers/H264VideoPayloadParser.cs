...
namespace Unity.LiveCapture.VideoStreaming.Client.MediaParsers
{
    class H264VideoPayloadParser : MediaPayloadParser
    {
        ...
        public override void Parse(TimeSpan timeOffset, ArraySegment<byte> byteSegment, bool markerBit)
        {
            ...
            //UnityDbg.Log($"H264VideoPayloadParser Parse {packMode}, {byteSegment.Count} bytes, marker: {markerBit}");
            switch (packMode)
            {
        ...
        private void ParseSTAP(ArraySegment<byte> byteSegment, int donFieldSize,
            bool markerBit)
        {
            //UnityDbg.LogFormat("ParseSTAP total sz {0}, marker {1} ==========================", byteSegment.Count, markerBit);
            Debug.Assert(byteSegment.Array != null, "byteSegment.Array != null");
            ...
            while (startOffset < endOffset)
            {
                ...
                //UnityDbg.LogFormat("ParseSTAP NALU sz {0} at {1}", nalUnitSize, startOffset + 2);
                if (nalUnitSize == 0)
                    break;
                startOffset += 2;
