...
namespace Unity.LiveCapture.VideoStreaming.Client.Rtp
{
    class RtpSequenceAssembler : IRtpSequenceAssembler
    {
        ...
        public void ProcessPacket(ref RtpPacket rtpPacket)
        {
            ...
            if (delta == 1)
            {
                //Debug.Log("ProcessPacket delta = 1");
                _previousCorrectSeqNumber = rtpPacket.SeqNumber;
                _previousTimestamp = rtpPacket.Timestamp;
                PacketPassed?.Invoke(ref rtpPacket);
                ...
                //Debug.Log($"ProcessPacket delta = 1 process {nextSeqNumber}");
            ...
            {
                //Debug.Log($"Start of new packet {rtpPacket.SeqNumber}, delta = {delta}, processing {_bufferedRtpPackets.Count} buffered packets.");
            ...
            else
            {
                ...
                {
                    //Debug.Log("ProcessPacket got twice the same sequence number!!!");
                    return;
                }
                ...
                //Debug.Log("ProcessPacket reached max correction length. Processing packets out of order.");
                PassNearestBufferedPacket();
