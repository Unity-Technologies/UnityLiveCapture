...
using Unity.LiveCapture.VideoStreaming.Client.MediaParsers;
using Unity.LiveCapture.VideoStreaming.Client.RawFrames;
using Unity.LiveCapture.VideoStreaming.Client.Rtcp;
using Unity.LiveCapture.VideoStreaming.Client.Rtp;
using Unity.LiveCapture.VideoStreaming.Client.Sdp;
using Unity.LiveCapture.VideoStreaming.Client.Tpkt;
...
namespace Unity.LiveCapture.VideoStreaming.Client.Rtsp
{
    sealed class RtspClientInternal : IDisposable
    {
        ...
        private async Task SetupTrackAsync(RtspMediaTrackInfo track, CancellationToken token)
        {
            ...
            if (_connectionParameters.RtpTransport == RtpTransportProtocol.UDP)
            {
                ...
                try
                {
                    ...
                    //UnityDbg.Log("Succeeded configuring UDP clients.");
                }
                catch
                {
                    UnityDbg.LogError("Failed configuring UDP clients.");
                    rtpClient.Close();
                    rtcpClient.Close();
                    throw;
                }
            ...
            if (_connectionParameters.RtpTransport == RtpTransportProtocol.UDP)
            {
                ...
                //UnityDbg.Log("Sending UDP hole punching packets.");
                ...
                //UnityDbg.Log("Sent UDP hole punching packets.");
            ...
            //UnityDbg.Log("UDP setup completed.0");
        }
            ...
            CancellationToken token)
        {
            ...
            var bufferSegments = new[] {bufferSegment};
            ...
            while (!token.IsCancellationRequested)
            {
                Profiler.BeginSample("Receive RTP/UDP");
                int peekRead = 0;
                try
                {
                    peekRead = client.Receive(peekBuffer, SocketFlags.Peek);
                }
                catch (SocketException)
                {
                    // ex.SocketErrorCode should be SocketError.WouldBlock, IOPending or NoBufferSpaceAvailable.
                    // Don't care for now: it's enough to know that peek could not proceed.
                }

                // Arbitrary limiting synchronous reads to 5, to avoid hogging the Synchronization context thread
                // for too long. For H.264 communications, the most degenerate case would be 3 packets per frame,
                // so assuming the execution context runs at the same frame rate as the stream emitter, 5 should be
                // good enough. But it'd be nicer to explore running the execution context from a thread that is
                // decoupled from Unity's main thread.
                int read = 0;
                if (peekRead > 0 && syncReadCount < 40)
                {
                    //UnityDbg.Log("Receiving RTP UDP packet synchronously.");
                    read = client.Receive(bufferSegments, SocketFlags.None);
                    ++syncReadCount;
                }
                else
                {
                    Profiler.EndSample();
                    //UnityDbg.Log("Receiving RTP UDP packet asynchronously.");
                    read = await client.ReceiveAsync(bufferSegment, SocketFlags.None);
                    Profiler.BeginSample("Receive RTP/UDP (async)");
                    syncReadCount = 0;
                }

                var payloadSegment = new ArraySegment<byte>(readBuffer, 0, read);

                Profiler.BeginSample($"RtpStream.Process {read} bytes");
                ...
                //UnityDbg.Log("Sending RTP UDP report.");
