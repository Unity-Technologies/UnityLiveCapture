...
namespace Zenject
{
    public abstract class ZenjectIntegrationTestFixture
    {
        ...
        // LiveCapture modification:
        // Add support for using this instance as an instance by another class.
        // (ValidateOnlyAttribute won't work but it's not useful to us right now.)
        bool CurrentTestHasAttribute<T>()
            where T : Attribute
        {
            var methodInfo = GetType().GetMethod(TestContext.CurrentContext.Test.MethodName);
            return methodInfo != null && methodInfo.GetCustomAttributes(true).Cast<Attribute>().OfType<T>().Any();
        }
